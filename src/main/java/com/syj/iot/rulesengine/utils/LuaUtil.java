...
public class LuaUtil {
	 ...
	 * @return
	 */
	private static Object getValueFromTable(final int type, final Varargs varargs, Object defaultValue,
			String... keylist) {
		Object result = null;
	...
	/**
	 * parse a luavalue to object
	 *
	 * @param type
	 * @param value
	 * @return
	 */
	public static Object Lua2Object(LuaValue value) {
		if (isBoolean(value))
			return value.checkboolean();
	...
	public static Varargs invokeFunction(LuaValue target, LuaValue arg1, LuaValue arg2, LuaValue arg3) {
		try {
			return (target != null && target.isfunction()) ? target.invoke(new LuaValue[] { arg1, arg2, arg3 })
					: LuaValue.NIL;
		} catch (Exception e) {
			e.printStackTrace();
			return LuaValue.NIL;
	...
	public static CharSequence toText(LuaValue luaValue) {
		/*
		 * if (LuaUtil.isString(luaValue)) { return luaValue.optjstring(null); } else if
		 * (LuaUtil.isUserdata(luaValue)) { if (luaValue instanceof UDSpannableString) {
		 * return ((UDSpannableString) luaValue).getSpannableStringBuilder(); } else if
		 * (luaValue instanceof UDUnicode) { return ((UDUnicode) luaValue).toString(); }
		 * }
		 */
		return null;
	}
